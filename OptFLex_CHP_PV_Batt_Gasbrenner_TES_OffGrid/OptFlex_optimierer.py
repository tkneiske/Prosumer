...
#==============================================================================
    # Operational costs
#     def obj_expression(model):
          return sum(model.P_CHP_el_exp[k] for k in model.K)\
                  +sum(model.P_PV_exp[k] for k in model.K)\
                  +sum(model.P_grid_imp[k] for k in model.K)
#                 
#==============================================================================
...
#     
  ...
  # model.OBJ = Objective(rule=obj_expression, sense=max)
    ...
    def ele_balance_constraint_rule(model,k):
         return model.P_Load[k] \
             - model.P_grid_imp[k]\
             + model.P_PV_exp[k]\
             + model.P_CHP_el_exp[k] \
             - model.P_PV_ava[k]  \
             + model.P_batt_char[k] \
             - model.P_batt_dis[k]\
             - model.P_CHP_el[k]  == 0
