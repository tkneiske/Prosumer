...
def Plotting(Result_Grid_End, Result_BAT_End, Result_PV_End,\
    PVavaPeriod, P_PV_max, P_Load_max, LoadPeriod, \
     Battery, Costs, date, date_end, Plotcounter, name):
#--------------------------------------
...
#--------------------------------------
    ...
    PVProfile = PVavaPeriod['PV 2013, Kassel, 10min']
    EProfile = LoadPeriod['ELoad']
    #QProfile = LoadPeriodFore['Q1Load']+LoadPeriodFore['Q2Load']
    PV2LoadWeek = Result_PV_End['PV load selfcon']
    GridLoadWeek = Result_Grid_End['Grid Import']
    Bat2LoadWeek = Result_BAT_End['Battery dis-charging']
    PVGridWeek = Result_PV_End['PV Grid export']
    PV2BatWeek = Result_PV_End['PV batt selfcon']
    socBat =  Result_BAT_End['SOC battery']
  #  Plotcounter = 0
    if plotEFlows == 1:
...
#                     else:
#                         fcolors=[CCHP2Load, CCHP2Bat]
#                         pltu.stackplot(x_axis,CHP32LoadWeek,colors=fcolors)
#                         plt.plot([], [], color=CCHP2Load, linewidth=10, label='CHP3-Direct consumption')
#                                    
#                     plt.ylabel('Power [kW]')
#                     plt.xticks(np.arange(0,LengthWeek/(Timesteps/365)+1, 1))
