...
#     #model.Np = Param(model.I, initialize = Np) #CHP
   ...
   # -------------- Costs ----------------------------------------     
    ...
    model.C_CHP_ex = Param(model.K, initialize= C_CHP_ex)
      # -------------- CO2 ----------------------------------------     
    model.CO2_gas = Param(model.K, initialize= CO2_gas)
    model.CO2_grid = Param(model.K, initialize= CO2_grid)
    model.CO2_PV = Param(model.K, initialize= CO2_PV)
...
#==============================================================================
       #==============================================================================
 #        Minimize CO2 Emissionkoeffizent x Leistung*deltaTime[h]
#     def obj_expression(model):
          return sum(model.CO2_gas[k]*model.P_aux_gas[k] for k in model.K)\
                  +sum(model.CO2_gas[k]*model.P_CHP_gas[k] for k in model.K)\
                  +sum(model.CO2_grid[k]*model.P_grid_imp[k] for k in model.K)\
                  +sum(model.CO2_PV[k]*model.P_PV_exp[k] for k in model.K)\
                  +sum(model.CO2_PV[k]*model.P_PV2batt[k] for k in model.K)\
                  +sum(model.CO2_PV[k]*model.P_PV2load[k] for k in model.K)#            
                  #+0.5*model.SOC_batt_term 
#==============================================================================
...
#==============================================================================
    ...
    #  --------------- Abregelung von 60% PV peak--------------------------------              
    if Abregel == 'ABon':
        def PV_Export3_rule(model,k):
            return model.P_PV_exp[k] <= model.P_PV_max[1]*0.6
        model.PV_Export3 = \
...
#    def CHP_min_OFF_rule1(model,k):
#        i = min(k+1, Np)        
#        return model.b_CHP_down[k] <= 1-model.b_CHP_on[i]
#    model.CHP_min_OFF1 = \
#    Constraint(model.K, rule=CHP_min_OFF_rule1)
#
#    def CHP_min_OFF_rule2(model,k):
#        i = min(k+2, Np)        
#        return model.b_CHP_down[k] <= 1-model.b_CHP_on[i]
#    model.CHP_min_OFF2 = \
#    Constraint(model.K, rule=CHP_min_OFF_rule2)
#    
#    def CHP_min_OFF_rule3(model,k):
#        i = min(k+3, Np)        
#        return model.b_CHP_down[k] <= 1-model.b_CHP_on[i]
#    model.CHP_min_OFF3 = \
#    Constraint(model.K, rule=CHP_min_OFF_rule3)
