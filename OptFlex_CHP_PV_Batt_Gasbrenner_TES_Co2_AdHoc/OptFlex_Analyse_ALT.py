...
# Colors for QFlows  
    ...
    for j in range(0, len(Saison)):
        for i in range(0, len(Horizon)):
         ...
         if i==0 and j==0:
            PrHoBin = Horizon[i]    
            KPI = pd.read_csv('Results\KPIsAdhoc'+str(name)+'_'+str(PrHoBin/6)+'_'+Saison[j]+'_10.csv')             
            # --------------------------------
            ...
            PrHoBin = Horizon[i]    
            KPIlocal = pd.read_csv('Results\KPIs'+str(name)+'_'+str(PrHoBin/6)+'_'+Saison[j]+'_10.csv')    
...
#    KPI['Autarcy [%]'].plot(kind='bar', title='Autarcy [%]',stacked=True, color=my_colors)
    ...
    plt.savefig('OUTPUT\AdHoc_BattCyc_'+str(name)+'.png', dpi=300)
    plt.figure()    
    KPI['CompTime [s]'].plot(kind='bar', title='Rechenzeit ',stacked=True, color=my_colors)
    plt.savefig('OUTPUT\AdHocV_Rechenzeit_'+str(name)+'.png', dpi=300)
...
#    KPI['GesamtSC [%]'].plot(kind='bar', title='GesamtSC [%]',stacked=True, color=my_colors)
    ...
    plt.savefig('OUTPUT\AdHoc_CHPruntime_'+str(name)+'.png', dpi=300)
    plt.figure()       
    KPI['totalCost [Euro]'].plot(kind='bar', title='totalCost [Euro]',stacked=True, color=my_colors)
    plt.savefig('OUTPUT\AdHoc_totalCost_'+str(name)+'.png', dpi=300)
    ...
    plt.savefig('OUTPUT\AdHoc_Costs_'+str(name)+'.png', dpi=300)
    ...
    plt.savefig('OUTPUT\AdHoc_PV_'+str(name)+'.png', dpi=300)
    ...
    plt.savefig('OUTPUT\AdHoc_CHP_'+str(name)+'.png', dpi=300)
    ...
    plt.savefig('OUTPUT\AdHoc_Q_Load_'+str(name)+'.png', dpi=300)
    ...
    plt.savefig('OUTPUT\AdHoc_E_Load_'+str(name)+'.png', dpi=300)
    ...
    plt.savefig('OUTPUT\AdHoc_Battery_Load_'+str(name)+'.png', dpi=300)
