...
#                                 + Result_CHP_End['CHP th']/CHP['eta_CHP_th']))*P2E   \
#                  +sum(Costs['C_grid_el']*Result_Grid_End['Grid Import'])*P2E   \
#                  -sum(Costs['C_CHP_FIT']*Result_CHP_End['CHP batt self']) *P2E  \
#                  -sum(Costs['C_CHP_ex']*Result_CHP_End['CHP batt self']) *P2E  \
#                  -sum(Costs['C_CHP_ex']*Result_CHP_End['CHP load self'])*P2E   \
#                  +sum(Costs['C_CHP_cs']*Result_CHP_End['CHP on_off']) *P2E  \
#                  -sum(Costs['C_PV_FIT']*Result_PV_End['PV Grid export'])*P2E    
    ...
    # Calculate Mean CHP-running time 
#    print Result_CHP_End['CHP on_off'][:maxx].values
#    y = np.diff(Result_CHP_End['CHP on_off'][:maxx].values)
#    print y
#    starts = (y>0.5).nonzero()
#    stops  = (y<-0.5).nonzero()
#    dur = stops[0]-starts[0]
#    print dur
#    meanCHPtime = np.mean(dur) * 10 # min
#    averageCHPtime = np.average(dur) * 10 #min
#    print meanCHPtime, averageCHPtime
    # --------------------------------------------------------------
    # PV
    # --------------------------------------------------------------
    ...
    totalCO2gas, totalCO2grid, totalCO2PV],\
    ...
    'totalCHP2TES [kWh]','CompTime Max [s]', 'totalCO2 [g]', 'totalCO2gas [g]', 'totalCO2grid [g]', 'totalCO2PV [g]'])
    print '--------------------------------'     
    print '  Key Performance Indicators:', PrHoBin
    print '--------------------------------'     
    print KPIs       
    KPIs.to_csv('Results\KPIsAdhoc'+str(name)+'_'+INOUT_string+'_10.csv')                                  
...
def Calc_KPI_dummy(name, PrHoBin,CompTime, INOUT_string):
    ...
    totalPV2Load, totalBattDis,totalTES2Load, totalBattchar,totalCHP2qLoad,totalCHP2TES, CompTime,\
    totalCO2, totalCO2gas, totalCO2grid, totalCO2PV],\
    index = ['totalCost [Euro]', 'totalGridExp [kWh]', 'totalGridImp [kWh]', 'totalPVExp [kWh]', \
    'totalCHPExp [kWh]','totalCHPelProd [kWh]', 'totalCHPthProd [kWh]', 'CHPsc [kWh]', 'PVsc [kWh]', \
    'totalPV [kWh]', \
    'totalEcon [kWh]', 'totalQcon [kWh]', 'totalAUX [kWh]', 'totalEH [kWh]', 'Autarcy [%]', \
    'totalEff ?', 'BatteryCyc ', 'CHPstarts ?', 'meanCHPtime ?', 'SelfconsumptionPV [%]',\
    'SelfconsumptionCHP [%]', 'GesamtSC [%]', 'CHPrunningTime [h]','totalCHP2BattCost',
    'totalCHP2eLoadCost [Euro]', 'totalPV2BattCost [Euro]', 'totalPV2LoadCost [Euro]',   
    'totalGasCost [Euro]', 'totalStromCost [Euro]', 'totalPVFeedInCost [Euro]', 
    'totalCHPFeedInCost [Euro]', 'totalCHPEigenCost [Euro]', 'totalCHPonofCost [Euro]',    
    'totalCHP2Batt [kWh]', 'totalCHP2eLoad [kWh]', 'totalPV2Batt [kWh]', \
    'totalPV2Load [kWh]', 'totalBattdis [kWh]', 'totalTES2Load [kWh]', 'totalBattchar [kWh]',\
    'totalCHP2qLoad [kWh]', 'totalCHP2TES [kWh]','CompTime [s]', 'totalCO2 [g]', \
    'totalCO2gas [g]', 'totalCO2grid [g]', 'totalCO2PV [g]'])
    print '--------------------------------'     
    print '  Key Performance Indicators:', PrHoBin
    print '--------------------------------'     
    print KPIs       
