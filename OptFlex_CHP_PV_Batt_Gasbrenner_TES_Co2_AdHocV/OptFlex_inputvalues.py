...
#      Nc = Nc_hrs*6 # Control horizon in 10 min interval
     ...
     eta_CHP_el, eta_CHP_th, b_CHP_on_ini, T_CHP_on, T_CHP_off],\
...
#     C_PV_FIT = C_PV_FIT*np.ones((Np,1))
     ...
     print Costs    
      ...
      # ---------------------------------------------    
     #       CO2 Emission Koeffizienten nach  VDI
     # ---------------------------------------------    
     CO2_gas = 202   # 200-550 g-kWh^-1
     #deutscher Strommix 2015 
     CO2_grid = 587   #g-kWh^-1
     CO2_PV = 60 # 50-60 g-kWh^-1CO2 = pd.Series([CO2_gas,CO2_grid, CO2_PV],\
...
#     C_KWK_Netznutz = 0.005 
     ...
     Battery.to_csv('OUTPUT\ParameterBattery_ohneFIT_MFH.csv')                        
     Costs.to_csv('OUTPUT\ParameterCosts_ohneFIT_MFH.csv')                                        
     CHP.to_csv('OUTPUT\ParameterCHP_ohneFIT_MFH.csv')      
     Auxilary.to_csv('OUTPUT\ParameterAux_ohneFIT_MFH.csv')   
     ThermalStorage.to_csv('OUTPUT\ParameterTES_ohneFIT_MFH.csv')        
     CO2.to_csv('OUTPUT\ParameterCO2_Default_MFH.csv')   
     
     return Battery, Auxilary,ThermalStorage, CHP, Costs
...
def Input_PV_Load_VDE_EFH(Delta_t, TimeStepSize,year_stamps):
     print " Use PV and Load from VDE Norm ! "
...
def ArtPreisProfileER(Delta_t, TimeStepSize,year_stamps, date_year, year_bins):      
    ...
    #-------------------  PV-Einspeisung ---------------     
     P=1
     PriceProfile[PriceProfile.index.hour<=4] = 2*P  # 0, -1
     PriceProfile[PriceProfile.index.hour>4] = 0*P  # 0, -1
     PriceProfile[PriceProfile.index.hour>11] = 0*P# 0, -1
     PriceProfile[PriceProfile.index.hour>14] = 0*P
     PriceProfile[PriceProfile.index.hour>16] = 0*P  # 0, -1
     PriceProfile[PriceProfile.index.hour>20] = 2*P  # 0, -1
