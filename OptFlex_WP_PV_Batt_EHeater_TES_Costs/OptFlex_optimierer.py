...
Load,P_PV_max_mat, P_Load_max_mat, P_sh_th, P_dhw_th, P_PV_ava, Battery,\
 ThermalStorage, EHeater, HeatPump, Costs):
   ...
   #T_CHP_off =  int(CHP['T_CHP_off'])
    eta_HP = {1: HeatPump['eta_HP']}
    P_HP_th_max = {1: HeatPump['P_HP_th_max']}
    P_HP_th_min = {1: HeatPump['P_HP_th_min']}
 #   b_HP_on_ini = {1: HP['b_HP_on_ini']}
 #   b_HP_ini_1 = {1: HP['b_HP_ini_1']}
 #   b_HP_ini_2 = {1: HP['b_HP_ini_2']}
 #   b_HP_ini_3 = {1: HP['b_HP_ini_3']}  

    #-----------------------------------------------------------------------
    #  -------------- E Heater ---------------------
    #----------------------------------------------------------------------- 
    eta_eheater = {1: EHeater['eta_eheater']}
    P_eheater_max = {1: EHeater['P_eheater_max']}
    P_eheater_min = {1: EHeater['P_eheater_min']}
...
#     #model.Np = Param(model.I, initialize = Np)
    ...
    #---------------- HeatPump ----------------------------------
    model.eta_HP = Param(model.I, initialize= eta_HP)
    model.P_HP_th_max = Param(model.I, initialize= P_HP_th_max )
    model.P_HP_th_min = Param(model.I, initialize=P_HP_th_min)
    # ------------ EHeater ---------------------------------
...
#    model.C_CHP_ex = Param(model.K, initialize= C_CHP_ex)
   ...
   # model.P_PV_sc = Var(model.K, domain=NonNegativeReals, bounds = (0, 100))
    ...
    model.P_PV2HP = Var(model.K, domain=NonNegativeReals, bounds = (0, 100))
    model.P_PV_exp = Var(model.K, domain=NonNegativeReals, bounds = (0,100))
    #-------------------------- Load ------------------------------
    model.P_load_imp = Var(model.K, domain=NonNegativeReals, bounds = (0,100))
    model.P_HP_imp = Var(model.K, domain=NonNegativeReals, bounds = (0,100))
    model.P_EHeater_imp = Var(model.K, domain=NonNegativeReals, bounds = (0,100))
    #-------------------------- Grid ------------------------------
    ...
    model.P_batt2HP = Var(model.K, domain=NonNegativeReals, bounds = (0,60))
    # ------------------ HeatPump--------------------------------
    model.P_HP_th = Var(model.K, domain=NonNegativeReals, bounds=(0,200))
    model.P_HP_el = Var(model.K, domain=NonNegativeReals, bounds=(0,200))
    model.b_HP = Var(model.K, domain=Binary)
    # ----------------  Thermal Storage -----------------------------------
...
#    Constraint(model.K, rule=ele_balance_constraint_rule)
    ...
    def ele_balance_constraint_rule(model,k):
         return model.P_Load[k] - model.P_grid_imp[k]\
             ...
             + model.P_eheater_el[k] + model.P_HP_el[k]== 0
    model.ele_balance_constraint = \
    Constraint(model.K, rule=ele_balance_constraint_rule)  
...
#    Constraint(model.K, rule=th_balance_constraint_rule)
    ...
    def th_balance_constraint_rule(model,k):
        return model.P_sh_th[k] + model.P_dhw_th[k] -  model.P_HP_th[k] \
...
#
    ...
    # HeatPump
    def PV_selfcons1HP_rule(model,k):
        return model.P_PV2HP[k] <= model.P_HP_el[k] 
    model.PV_selfcons1HP = \
    Constraint(model.K, rule=PV_selfcons1HP_rule)

    def PV_selfcons2HP_rule(model,k):
        #return model.P_PV2load[k] + model.P_PV2eheater_el[k]<= model.P_PV_ava[k]        
        return model.P_PV2HP[k] <= model.P_PV_ava[k]        
    model.PV_selfcons2HP = \
    Constraint(model.K, rule=PV_selfcons2HP_rule)
    ...
    def PV_Export1_rule(model,k):
        return model.P_PV_exp[k] == P_PV_ava[k] - model.P_PV2load[k] -\
                                    model.P_PV2batt[k] - model.P_PV2HP[k] -\
                                    model.P_PV2Eheater[k]
    model.PV_Export1 = \
...
# --------------------- Load ------------------------------------------------
#-----------------------------------------------------------------------------
    def HPLoad_rule(model,k):
        return model.P_HP_el[k] == model.P_HP_imp[k] + model.P_batt2HP[k] + model.P_PV2HP[k]
    model.HPLoad = \
    Constraint(model.K, rule=HPLoad_rule)

    def EHeaterLoad_rule(model,k):
        return model.P_eheater_el[k] == model.P_EHeater_imp[k] + model.P_batt2Eheater[k] + model.P_PV2Eheater[k]
    model.EHeaterLoad = \
    Constraint(model.K, rule=EHeaterLoad_rule)

    def Load_rule(model,k):
        return  model.P_load_imp[k] == model.P_Load[k] - (model.P_batt2Load[k] + model.P_PV2load[k])
    model.Load = \
    Constraint(model.K, rule=Load_rule)
...
#------------------------------------------------------------------------------
    ...
    def Grid_imp_rule(model,k):
       return model.P_grid_imp[k] == model.P_load_imp[k] + model.P_HP_imp[k] + model.P_EHeater_imp[k]   
    model.Grid_imp = \
    Constraint(model.K, rule=Grid_imp_rule)     
    ...
    #    (1-model.b_batt[k]) <= 0
       #return model.P_grid_imp[k] - (model.P_grid_load[k]+ model.P_eheater_el[k]+ model.P_HP_el[k])*\
       return model.P_grid_imp[k] - (model.P_Load_max[1]+ model.P_eheater_max[1]+ model.P_HP_th_max[1]/eta_HP[1])*\
       (1-model.b_batt[k]) <= 0
...
#------------------------------------------------------------------------------
     ...
     # Equation(??)
    def HP_Modulation1_rule(model,k):
        return model.P_HP_th[k] <= model.P_HP_th_max[1]*model.b_HP[k]
    model.HP_Modulation1 = \
    Constraint(model.K, rule=HP_Modulation1_rule)
 
    # Equation(??)
    def HP_Modulation2_rule(model,k):
        return model.P_HP_th[k] >= model.P_HP_th_min[1]*model.b_HP[k]     
    model.HP_Modulation2 = \
    Constraint(model.K, rule=HP_Modulation2_rule)
     
    # Equation(??)
    def HP_Performance_rule(model,k):
        return model.P_HP_el[k] == model.P_HP_th[k]/model.eta_HP[1]   
    model.HP_Performance = \
    Constraint(model.K, rule=HP_Performance_rule)
 
   # def HP_Import1_rule(model,k):
   #      return model.P_HP_imp[k] == model.P_HP_el[k] - model.P_PV2HP[k]
   # model.HP_Import1 = \
   # Constraint(model.K, rule=HP_Import1_rule)

    def HP_Import2_rule(model,k):
         return model.P_HP_imp[k] == model.P_grid_imp[k] - model.P_load_imp[k] - model.P_EHeater_imp[k]
    model.HP_Import2 = \
    Constraint(model.K, rule=HP_Import2_rule)
...
#------------------------------------------------------------------------------
    ...
    def TES_char_rule(model,k):
       ...
       - model.P_HP_th[k] - model.P_eheater_th[k]== 0  
...
#       - model.P_CHP_th[k] == 0  
    ...
    P_load_imp_res = np.zeros(PrHoBin)
    varobject = getattr(instance, 'P_load_imp')
    for i in range(0, PrHoBin):
        P_load_imp_res[i] = varobject[(i)].value
        #print P_grid_imp_res
    P_load_imp_res_df = pd.DataFrame(
                        P_load_imp_res, index=horizon_stamps, columns=['P_load_imp'])
    ...
    P_batt2Load_res = np.zeros(PrHoBin)
    varobject = getattr(instance, 'P_batt2Load')
    for i in range(0, PrHoBin):
        P_batt2Load_res[i] = varobject[(i)].value
    P_batt2Load_res_df = pd.DataFrame(
                        P_batt2Load_res, index=horizon_stamps, columns=['P_batt2Load'])
                        
    P_batt2Eheater_res = np.zeros(PrHoBin)
    varobject = getattr(instance, 'P_batt2Eheater')
    for i in range(0, PrHoBin):
        P_batt2Eheater_res[i] = varobject[(i)].value
    P_batt2Eheater_res_df = pd.DataFrame(
                        P_batt2Eheater_res, index=horizon_stamps, columns=['P_batt2Eheater'])
                        
    P_batt2HP_res = np.zeros(PrHoBin)
    varobject = getattr(instance, 'P_batt2HP')
    for i in range(0, PrHoBin):
        P_batt2HP_res[i] = varobject[(i)].value
    P_batt2HP_res_df = pd.DataFrame(
                        P_batt2HP_res, index=horizon_stamps, columns=['P_batt2HP'])                        
#==============================================================================
...
#==============================================================================
    ...
    P_PV2HP_res = np.zeros(PrHoBin)
    varobject = getattr(instance, 'P_PV2HP')
    for i in range(0, PrHoBin):
        P_PV2HP_res[i] = varobject[(i)].value
    P_PV2HP_res_df = pd.DataFrame(
                        P_PV2HP_res, index=horizon_stamps, columns=['P_PV2HP'])                        
    ...
    # ----------------------- HeatPump ------------   
    P_HP_th_res = np.zeros(PrHoBin)
    varobject = getattr(instance, 'P_HP_th')
    for i in range(0, PrHoBin):
        P_HP_th_res[i] = varobject[(i)].value
    P_HP_th_res_df = pd.DataFrame(
                        P_HP_th_res, index=horizon_stamps, columns=['P_HP_th'])

    P_HP_el_res = np.zeros(PrHoBin)
    varobject = getattr(instance, 'P_HP_el')
    for i in range(0, PrHoBin):
        P_HP_el_res[i] = varobject[(i)].value
    P_HP_el_res_df = pd.DataFrame(
                        P_HP_el_res, index=horizon_stamps, columns=['P_HP_el'])
  
    P_HP_imp_res = np.zeros(PrHoBin)
    varobject = getattr(instance, 'P_HP_imp')
    for i in range(0, PrHoBin):
        P_HP_imp_res[i] = varobject[(i)].value
    P_HP_imp_res_df = pd.DataFrame(
                        P_HP_imp_res, index=horizon_stamps, columns=['P_HP_imp'])
    ...
    P_EHeater_imp_res = np.zeros(PrHoBin)
    varobject = getattr(instance, 'P_EHeater_imp')
    for i in range(0, PrHoBin):
        P_EHeater_imp_res[i] = varobject[(i)].value
    P_EHeater_imp_res_df = pd.DataFrame(
                        P_EHeater_imp_res, index=horizon_stamps, columns=['P_EHeater_imp'])
...
#-----------------------------------------------------------------------------
    ...
    P_PV2Eheater_res_df, P_eheater_th_res_df, P_eheater_el_res_df, \
    P_PV2HP_res_df, P_HP_th_res_df, P_HP_el_res_df,
    P_batt2Eheater_res_df,P_batt2Load_res_df,P_batt2HP_res_df,\
     P_HP_imp_res_df, P_load_imp_res_df,P_EHeater_imp_res_df\
    ], axis=1)   
 
   # print OptResult['P_PV2HP'][OptResult.index[0]],OptResult['P_HP_el'][OptResult.index[0]],OptResult['P_HP_th'][OptResult.index[0]]  
    #print OptResult['P_HP_imp'][OptResult.index[0]],OptResult['P_PV2HP'][OptResult.index[0]],OptResult['P_HP_el'][OptResult.index[0]]
    return OptResult
