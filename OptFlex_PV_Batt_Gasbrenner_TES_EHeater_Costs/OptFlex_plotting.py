...
#    print NoYes
    ...
    QStor2QLoadWeek = QProfile#NGBurner2QLoadWeek 
...
#  Load on Grid                    
            if NoYes['Load'] == 1:  
              ...
              #      *Timesteps/8760.+EEHeatQStorWeek,label='E-Load, HH (inc. e-heating)',linewidth=0.7)
                    ...
                    plt.plot(x_axis,EProfile,label='E-Load, HH',linewidth=0.7)                                 
                if NoYes['GridLoad'] == 1: 
                     if NoYes['PV'] == 1 and NoYes['CHP'] == 0 and NoYes['EStor'] == 1:
...
#-----------------------------------------------------------------------------                
    ...
    #             if NoYes['NGBurner'] == 1 and NoYes['CHP'] == 1 and NoYes['QStor'] == 1 and NoYes['EHeatQStor'] == 1:
    #                 fcolors=[CCHP2QLoad, CNGBurner2QLoad, CQStor2QLoad,CEHeat2QStor]
    #                 plt.stackplot(x_axis,CHP2QLoadWeek,NGBurner2QLoadWeek,QStor2QLoadWeek,EHeatQStor2QStorWeek,colors=fcolors)  
    ...
    #                 plt.plot([], [], color=CEHeat2QStor, linewidth=10, label='E. heater-storage charging')
                 if NoYes['NGBurner'] == 1 and NoYes['CHP'] == 0 and NoYes['QStor'] == 1:
    ...
    if SubPlots5 > 0:
             ...
             if NoYes['QStor'] == 1:                           
               #  if max(QStorD) >= 0.005:
                     ...
                     if NoYes['NGBurner'] == 1 and NoYes['CHP'] == 0 and NoYes['EHeatQStor'] == 0:
                    ...
                    #     plt.plot([], [], color=CNGBurner2QStor, linewidth=10, label='NG burner-storage charging')
                         ...
                         plt.stackplot(x_axis,-QStor2QLoadWeek,colors=fcolors1) 
                         plt.stackplot(x_axis,EHeatQStor2QStorWeek,NGBurner2QStorWeek,colors=fcolors2)   
                        # plt.plot([], [], color=CNGBurner2QStor, linewidth=10, label='NG burner-storage charging')
                         #plt.plot([], [], color=CEHeat2QStor, linewidth=10, label='E. heater-storage charging')
                         plt.plot([], [], color=CQStor2QLoad, linewidth=10, label='Q-storage-load') 
                         plt.ylabel('Power [kW]')
